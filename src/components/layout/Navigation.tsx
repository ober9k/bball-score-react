...
const links: Array<{ id: number, title: string, to: string }> = [
  ...
  { id: linkId++, title: "Statistics", to: Paths.Statistics, },
  ...
  return (
    <div className="bg-gray-800 flex h-16 items-center justify-between">
      <div className="flex items-center">
        <div className="shrink-0 pl-4 pr-4 py-2">
        ...
        <div className="">
          {links.map((link) => (
            <Link key={link.id} to={link.to} className="[&.active]:font-bold rounded-md px-2 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">
              {link.title}
            </Link>
          ))}
        </div>
