...
import {
  ...
  gamesQueryOptions,
  playersQueryOptions,
  seasonsQueryOptions, statisticsQueryOptions,
  teamsQueryOptions
...
export async function statisticsLoader({ context }) {
  return fetchAllWithConverter<StatisticsLog>(context.queryClient, statisticsQueryOptions, toStatisticsLog);
}
